                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                @if($report->status === 'sent')
                                    {{-- Archivieren --}}
                                    <form action="{{ route('reports.archive-report', $report) }}" method="POST" class="inline" x-data
                                          @submit.prevent="confirmAction($el, 'Bericht archivieren', 'Möchten Sie diesen Bericht archivieren?', 'Archivieren')">
                                        @csrf
                                        <button type="submit" class="p-1.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded" title="Archivieren">
