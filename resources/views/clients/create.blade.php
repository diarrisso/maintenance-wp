    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-lg shadow-sm">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900">Kunde erstellen</h3>
            </div>
            ...
            <form action="{{ route('clients.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="maintenance_type"
                        ...
                        @foreach(\App\Models\Client::MAINTENANCE_TYPES as $value => $label)
                            <option value="{{ $value }}" {{ old('maintenance_type', '1x_monthly') === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="notes"
                              id="notes"
                              rows="4"
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
